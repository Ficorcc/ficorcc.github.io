...
{#each routes as item}
	...
	<a href={item.path} onclick={handleClose} class="relative inline-flex items-center justify-center group" class:max-sm:font-bold={isActive}>
			<span class="sm:absolute sm:w-full h-full inline-flex items-center justify-center sm:justify-center sm:border-b-2 sm:py-1 transition-[border-color] duration-150 ease-linear" class:border-transparent={!isActive} class:border-secondary={isActive}><Icon name={item.icon} size={20} class="inline-block" /></span>
